...
class StaffCardExportTest extends TestCase
{
    ...
    public function test_pdf_html_has_exact_mm_layout_page_limits_vietnamese_font_and_vector_qr(): void
    {
        ...
        $horizontalHtml = $exports->pdfHtml(collect(array_fill(0, 21, $card)), 'horizontal');
        ...
        $this->assertStringContainsString('height: 24mm', $horizontalHtml);
        $this->assertSame(2, substr_count($horizontalHtml, '<section class="pdf-page'));
        $this->assertStringContainsString('size: A5 portrait', $verticalHtml);
        $this->assertStringContainsString('width: 148mm', $verticalHtml);
        $this->assertStringContainsString('height: 210mm', $verticalHtml);
        $this->assertSame(5, substr_count($verticalHtml, '<section class="pdf-page'));
        ...
        $horizontalPdf = $exports->renderPdf(collect(array_fill(0, 21, $card)), 'horizontal');
        ...
        $this->assertSame(5, preg_match_all('/\/Type\s*\/Page\b/', $verticalPdf));
        $this->assertMatchesRegularExpression(
            '/\/MediaBox\s*\[\s*0(?:\.0+)?\s+0(?:\.0+)?\s+595\.\d+\s+841\.\d+\s*\]/',
            $horizontalPdf
        );
        $this->assertMatchesRegularExpression(
            '/\/MediaBox\s*\[\s*0(?:\.0+)?\s+0(?:\.0+)?\s+419\.\d+\s+595\.\d+\s*\]/',
            $verticalPdf
        );
    }
    ...
    public function test_both_templates_download_as_zip_with_two_valid_pdfs(): void
    {
        ...
        $this->assertStringStartsWith('%PDF-', $zip->getFromName('the-nhan-su-ngang-80x24.pdf'));
        $this->assertStringStartsWith('%PDF-', $zip->getFromName('bang-danh-gia-nhan-su-a5.pdf'));
        $zip->close();
        @unlink($path);
    }
    ...
    public function test_missing_optional_data_and_duplicate_last_four_digits_export_safely(): void
    {
        ...
        $this->assertTrue($cards->every(
            fn (array $card) => str_starts_with($card['branch_logo_url'], 'data:image/png;base64,')
                && str_starts_with($card['maxsim_logo_url'], 'data:image/png;base64,')
        ));
    ...
    public function test_long_vietnamese_name_is_clipped_inside_fixed_size_cards(): void
    {
        ...
        $this->assertStringContainsString('font-size: 6.1pt', $horizontalHtml);
        $this->assertStringContainsString('white-space: normal', $horizontalHtml);
        $this->assertStringNotContainsString('max-height: 12.5mm', $horizontalHtml);
        $this->assertStringNotContainsString('max-height: 31mm', $verticalHtml);
        $uppercaseName = mb_strtoupper($card['name'], 'UTF-8');
        $this->assertSame(1, substr_count($horizontalHtml, $uppercaseName));
        $this->assertSame(1, substr_count($verticalHtml, $uppercaseName));
    ...
    public function test_requested_vietnamese_names_render_without_clipping_rules(): void
    {
        $exports = app(StaffCardExportService::class);
        $names = [
            'NGUYỄN TẤN THỜI',
        ...
        $cards = collect($names)->map(fn (string $name) => [
            'name' => $name,
            'position' => 'PHỤC VỤ',
            'avatar_url' => null,
            'branch_logo_url' => '',
        ...
        $horizontalHtml = $exports->pdfHtml($cards, 'horizontal');

        foreach ($names as $name) {
            $this->assertSame(1, substr_count($horizontalHtml, $name));
        }

        $this->assertDoesNotMatchRegularExpression(
            '/\.horizontal-name\s*\{[^}]*overflow\s*:\s*hidden/s',
            $horizontalHtml
        );
        $this->assertStringStartsWith('%PDF-', $exports->renderPdf($cards, 'horizontal'));
    }
}
