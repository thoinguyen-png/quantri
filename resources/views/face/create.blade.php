            ...
            <div class="face-flow-header">
                ...
                <p>
                    @if ((auth()->user()->face_verification_mode ?? 'normal') === 'priority')
                        Giữ khuôn mặt nhìn thẳng trong khung hình. Hệ thống sẽ tự nhận diện và lưu mẫu gương mặt.
                    @else
                        Hệ thống sẽ tự mở camera trước, nhận diện 1 khuôn mặt rõ ràng và lưu mẫu gương mặt cho tài khoản của bạn.
                    @endif
                </p>
            </div>
        ...
        async function detectAndRegisterFace(runId) {
            ...
            if (FACE_VERIFICATION_MODE === 'priority') {
                if (direction !== 'center') {
                    descriptorSamples = [];
                    step = 'center';
                    setStatus('Vui lòng nhìn thẳng vào camera.', 'yellow');
                    setTimeout(() => detectAndRegisterFace(runId), 700);
                    return;
                }
                ...
                if (step !== 'samples') {
                    descriptorSamples = [];
                    step = 'samples';
                    setStatus('Đang nhận diện khuôn mặt. Giữ mặt thẳng và ổn định...', 'yellow');
                }
            }
            ...
            if (step === 'center') {
                ...
                if (direction === 'center') {
                    step = 'left';
                    setStatus('Tốt. Bây giờ quay mặt sang TRÁI.', 'yellow');
                } else {
            ...
            if (step === 'left') {
                ...
                if (direction === 'left') {
                    step = 'right';
                    setStatus('Tốt. Bây giờ quay mặt sang PHẢI.', 'yellow');
                } else {
                    setStatus('Vui lòng quay mặt sang TRÁI.', 'yellow');
                }
            ...
            if (step === 'right') {
                ...
                if (direction === 'right') {
                    step = 'final';
                    setStatus('Tốt. Nhìn thẳng lại để hoàn tất.', 'yellow');
                } else {
                    setStatus('Vui lòng quay mặt sang PHẢI.', 'yellow');
                }
            ...
            if (step === 'final') {
                ...
                if (direction === 'center') {
                    step = 'samples';
                    setStatus('Đã nhận diện khuôn mặt. Giữ yên để lưu...', 'green');
                } else {
                    setStatus('Vui lòng nhìn thẳng lại camera.', 'yellow');
                    setTimeout(() => detectAndRegisterFace(runId), 700);
                    return;
                }
            }
            ...
            if (descriptor.length < 128 || descriptor.some(value => !Number.isFinite(value))) {
                descriptorSamples = [];
                setStatus('Dữ liệu khuôn mặt chưa hợp lệ. Vui lòng nhìn thẳng và thử lại.', 'yellow');
                setTimeout(() => detectAndRegisterFace(runId), 700);
                return;
            }

            descriptorSamples.push(descriptor);
            ...
            const averagedDescriptor = averageDescriptors(descriptorSamples.slice(-REQUIRED_SAMPLES));
            await submitFaceDescriptor(averagedDescriptor);
        }
