<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login', absolute: false) }}">
        @csrf
        <input type="hidden" name="client_context" value="browser" data-client-context>

        <div>
            <x-input-label for="email" value="Email hoặc SĐT" />
    ...
    <script>
        (function () {
            const input = document.querySelector('[data-client-context]');
            ...
            input.value = standalone ? 'pwa' : 'browser';
        })();
    </script>
</x-guest-layout>
